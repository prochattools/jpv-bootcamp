...
export function portalPageHtml(section: 'home' | 'billing-checkout' | 'billing-active' | 'account' | 'referral' | 'support'): string {
  const content = {
    ...
    'billing-checkout': '<h1>Billing</h1><p>Choose a supported Pro membership option.</p><button data-billing="monthly">£80/month</button><button data-billing="annual">£800/year</button><p>No minimum commitment.</p><section><h2>Billing projection summary</h2><p>Projected plan: Free</p></section>',
    'billing-active': '<h1>Billing</h1><p>Manage your subscription, invoices, and payment methods through our secure billing portal.</p><section><h2>Subscription status</h2><p>Current plan</p><p>Pro membership</p><p>Billing cadence</p><p>Monthly billing</p><p>Commitment state</p><p>Active</p></section><section><h2>Manage subscription</h2><button type="button">Manage billing</button><button type="button">Request end-of-term cancellation</button></section><section><h2>Billing projection summary</h2><p>Projected plan: Pro</p><p>Projected subscription status: Active</p></section>',
