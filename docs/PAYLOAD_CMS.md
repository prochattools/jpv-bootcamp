...
`/learn/login`, `/learn`, `/learn/account`, `/learn/[courseSlug]`, `/learn/[courseSlug]/[lessonSlug]`, `/learn/resources/[resourceId]`, `/learn/community`, and `/learn/community/[spaceSlug]` are dynamic Node routes backed by `payload_members`. Public member signup is still disabled; member creation remains controlled by Payload admin, Stripe shadow sync, or migration flows. Account-security source flows now cover invitation, email verification, password reset/change, pending email change, blocked/restored notices, queued delivery, and audit; runtime use still requires the separate migration, provider-email, and deployment approvals documented in `docs/PREVIEW_RELEASE_READINESS.md`. Lesson rich text, media players, comments, community posting, moderation actions, and chat are not live yet.
