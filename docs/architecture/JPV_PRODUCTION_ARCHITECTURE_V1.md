...
| Production image tag | `a287800735d465a41ad9e45d2c7914ab9cc34a26` | Publish workflow `34398931497` and post-deployment health probe. |
| GitHub Actions | Publish run `34398931497` passed; read-only migration-status run `34147184195` is historical evidence | Publish passed source verification, build, immutable image publication, Dokploy update, deployment trigger, and convergence; no separate production migration workflow ran. |
| Deployment | Serving image tag observed after successful publish and Dokploy convergence | Publish run `34398931497` and post-deployment health probe. |
| Live identity | `status=live`; `deploymentEnv=production`; image tag `a287800735d465a41ad9e45d2c7914ab9cc34a26` | Post-deployment health probe. |
