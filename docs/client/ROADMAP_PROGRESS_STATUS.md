...
Approve table-plan-to-Free mapping for target environment before migrations, then perform approved migration rehearsal.
...
| Latest validated baseline before checkout hardening | `80012b7 fix: staging readiness audit — access semantics, shadow sync, docs, and test fixes` |
...
| --- | ---: | ---: | ---: | --- | --- |
| Overall expanded platform | ~58% | ~65% | +7% | Free/Pro refit committed/pushed/clean; legacy paths removed; billing hardening; focused validation passed; Prisma and tsc validated | Live cutover unapproved; migrations unapplied; production content incomplete; provider email live verification pending |
| Core staging readiness | ~68% | ~90% | +22% | Branch pushed, clean, validated, hardened; shadow sync fixed; sponsored access corrected; docs hardened; Dokploy API key literal removed; migration inventory unified to 11; checkout legacy-plan rejection and return URL safety covered | Migration approval; post-refit staging smoke; provider email live verification |
...
| Testing / release readiness | ~70% | ~83% | +13% | Focused checkout validation coverage added; billing, entitlement, course access, shadow sync, sponsored claim/decision helpers covered | Batch-runner timing artifact (tests pass individually); migration rehearsal not executed |
...
1. Approve the table-plan-to-Free mapping for the target environment.
2. Perform approved migration rehearsal through the approved database migration path.
