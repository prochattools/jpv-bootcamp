...
2. **Phase 9 — LiveKit Group Calls** — **DEPLOYED TO STAGING 2026-08-21. LiveKit READY FOR ACCEPTANCE.** Space-based group call model fully implemented and deployed. Migration 36 (`20260820_000000_live_session_space`) applied. LiveKit env vars (`LIVEKIT_URL`, `LIVEKIT_API_KEY`, `LIVEKIT_API_SECRET`) configured in Dokploy staging app. Token endpoint `/api/livekit/token` operational and auth-gated. **Current deployed SHA:** `9c0debe3bdf0fc5a9c9be99a6697eb6bbff3419d` (deploy run `32462177363`). **Deployment convergence fix (`9c0debe`):** wait step now retriggers Dokploy deploy at attempt 10 and force-redeploys at attempt 25 if Docker Swarm fails to converge — eliminates the manual `docker service update` workaround needed on 2026-08-20. **GHCR auth resolved:** `GHCR_PAT` secret in `preview-deploy` environment, host Docker daemon credentialed, all pulls succeed. **Migration status:** 36/36 applied, 0 pending. **Portal performance:** community pages ~15-20s networkidle (down from ~60s) via `withQueryDedup` + batch membership pre-fetch (70-80% query reduction). **LiveKit verification (2026-08-21):** (1) unauthenticated POST → `unauthorized` ✓; (2) authenticated admin POST with nonexistent session → `session_not_found` ✓ (proves config loaded, no `server_misconfigured`); (3) all 3 LiveKit env vars confirmed present in Docker service spec. **Remaining for acceptance:** create a test `live_session` record, verify authenticated entitled member gets a valid JWT, two-participant call test. **Release gate:** 164/164 passing.
