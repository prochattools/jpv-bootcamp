...
2. **Phase 9 — LiveKit Group Calls** — **IN PROGRESS** 2026-08-20. Space-based group call model implemented: `live_sessions` extended with optional `space` FK (migration `20260820_000000_live_session_space`), space membership authorization on `/api/livekit/token` POST, `canPublish: true` for all group call participants, `LiveCallRoom` React client component using `@livekit/components-react` `VideoConference`, calls list and join pages in community portal, 7 new authorization tests pass. Commit pending deploy.
