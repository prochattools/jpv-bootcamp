...
`34392897174` passed source verification, application validation, immutable
image publication, Dokploy update, deployment trigger, and production
convergence for `main` commit/image
`d55ac6a30bcd78c63bd2a0a46db709617efc48f2`. The live deployment-health
...
- **NEXT PHASE RANKING:** (1) Phase 8 — Member Portal Operationalization (**COMPLETE** 2026-08-20, SHA `9bd35c0`, deploy run `32384925382`); (2) Phase 9 — LiveKit Group Calls (**COMPLETE** 2026-08-21 — backend + browser acceptance 5/5 PASS; see Phase 9 entry below); (3) Phase 10 — production cutover only under separate explicit authorization; (4) Phase 11 — Partner Affiliates deferred post-cutover. **Non-blocking open item:** `DEPLOY_SSH_HOST`/`DEPLOY_SSH_USER` not in `preview-deploy` GitHub environment — SSH fallback skipped but Dokploy retrigger-at-attempt-10 still active; add secrets when convenient but not a blocker for Phase 10.
...
2. **Phase 9 — LiveKit Group Calls** — **COMPLETE 2026-08-21. LiveKit backend + browser acceptance PASSED.** Space-based group call model fully implemented, deployed, and acceptance-verified. Migration 36 (`20260820_000000_live_session_space`) applied. LiveKit env vars (`LIVEKIT_URL`, `LIVEKIT_API_KEY`, `LIVEKIT_API_SECRET`) configured. **Current deployed SHA:** `9c0debe3bdf0fc5a9c9be99a6697eb6bbff3419d` (deploy run `32462177363`). **LiveKit backend acceptance evidence (2026-08-21):** (1) Anonymous POST `/api/livekit/token` → HTTP 401 `unauthorized` ✓; (2) Admin (host, `info@prochat.tools`) POST → HTTP 200 with JWT: `canPublish:true`, `roomAdmin:true`, `room:space-1-staging-qa-001`, `wsUrl:wss://jpv-bootcamp-8wi8xcoy.livekit.cloud` ✓; (3) QA member (`staging-qa-auth@prochat.tools`, active space membership) POST → HTTP 200 with JWT: `canPublish:true` (space call), `roomAdmin:null` ✓; (4) LiveKit cloud room `space-1-staging-qa-001` created and verified via server SDK (SID: `RM_Su53GS9G5...`) ✓. **LiveKit browser acceptance evidence (2026-08-21):** Playwright `portal-calls-acceptance.staging.spec.ts` 5/5 PASS — CALLS-001 unauthenticated redirect ✓; CALLS-002 authenticated calls list shows live session with "Live now" badge and "Join call" link ✓; CALLS-003 join call page renders `LiveCallRoom` with "Join call" button, no JS errors ✓; CALLS-004 "Join call" button POSTs to `/api/livekit/token`, receives `{ok:true, token, wsUrl, roomName:space-1-staging-qa-001}`, LiveKit `CallStage` component mounts ✓; CALLS-005 unauthenticated join call page redirects to login ✓. **Human validation remaining (non-blocking):** actual WebRTC AV stream, two-participant simultaneous call, leave-call disconnect — require real browser with device permissions. **QA test data (staging-only):** live_session id=1, room `space-1-staging-qa-001`, status=live, space_id=1; space_membership id=113 for member id=51 (`staging-qa-auth@prochat.tools`). **Portal performance:** community pages ~130ms (down from ~60s). **Deployment convergence fix:** wait step retriggers Dokploy at attempt 10; SSH fallback at attempt 18 if `DEPLOY_SSH_HOST` is set. **Release gate:** 164/164 passing.
...
| **Latest verification snapshot** | 2026-09-09 post-deployment read-only runtime identity refresh: staging image/commit `8b1f459fed358776fda791553ef225cc9f03b2ae`, production image/commit `d55ac6a30bcd78c63bd2a0a46db709617efc48f2`. See `docs/release/LIVE_RUNTIME_IDENTITY_REFRESH_2026-09-09.md`. |
