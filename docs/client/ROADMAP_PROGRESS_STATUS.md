...
| **Current CODE HEAD** | `9e6af02 docs: record transactional email logo verification and live staging proof` (2026-07-29 — 156/156 release tests, support workflow complete, transactional email logo live on staging); verify with `git log --oneline -1` before any operator action |
...
- **Operator dashboard hardening**: `pnpm test:release` passed `156/156` (2026-07-27 — sidebar groups consolidated (Members, Emails, Community, System); member security events hidden from sidebar; CRM/admin collections grouped as Emails; route-integrity test added verifying all dashboard links target real collection slugs and no developer-only links remain).
