...
2. **Phase 9 — LiveKit Group Calls** — **CODE COMPLETE 2026-08-20 — STAGING MIGRATION AND DEPLOY PENDING.** Space-based group call model fully implemented: `live_sessions` extended with optional `space` FK (migration `20260820_000000_live_session_space`, registry entry #36 of 36), space membership authorization on `/api/livekit/token` POST, `canPublish: true` for all group call participants, `roomAdmin: true` for host, `LiveCallRoom` React client component using `@livekit/components-react` `VideoConference`, calls list and join pages in community portal, 7 new authorization tests + 4 lifecycle tests pass. Push gate CI run `32397679498` green (TypeScript, build, Prisma, release gate 164/164, E2E 148/188). Migration runner and workflow updated for Phase 9 batch (migration 36 pending apply to staging). Next operator actions: (a) trigger deploy of SHA `3c5e46456acd42f49b51e9a09abc69eca2886bc5` to `clients-jpv-bootcamp-app-tp9xrk`; (b) run `pnpm staging:payload-migration-plan` (GH Actions `read-only-migration-plan`) to confirm 35 applied / 1 pending; (c) authorized apply of migration 36 to `jpvbootcamp_staging`; (d) verify LiveKit config (`LIVEKIT_URL`, `LIVEKIT_API_KEY`, `LIVEKIT_API_SECRET`) in staging environment.
...
**Staging acceptance:** Phase 8 green. Phase 9 code complete; staging migration (#36 `live_session_space`) and Phase 9 deploy are the remaining steps before Phase 9 staging acceptance.
