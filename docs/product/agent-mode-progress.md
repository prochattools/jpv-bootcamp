...
## Authoritative staging technical checkpoint — 2026-08-02

### Deployed staging state

- **Deployed SHA:** `34c6e7550bd2103a05c4414a2f582787dc15bf08`
- **Preview CI workflow:** `30719336990` concluded `success` for the exact SHA
...
- **Media persistence:** verified via named volume and disposable fixture survival across redeployment
...
| Release suite | 163/163 PASS, including account-action hardening-status guard |
| Browser E2E | 148/148 PASS (40 skipped or excluded) |
| Admin responsive (CI) | PASS |
...
- **Technical implementation:** `STAGING TECHNICAL IMPLEMENTATION COMPLETE — ACCEPTANCE PENDING EXTERNAL ACTION`
- **Hardening:** `STAGING HARDENING REMEDIATION REQUIRED`
- **Open repository issue:** one-time account actions do not yet have a durable cross-instance reservation/finalization state. Email-change confirmation consumes before its member update succeeds; invitation and password-reset flows validate before mutation but do not serialize concurrent downstream mutation.
