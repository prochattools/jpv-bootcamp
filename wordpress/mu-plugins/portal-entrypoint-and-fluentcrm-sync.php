...
function jpv_portal_provision_get_token(): string {
    ...
    if (defined('WP_PROVISION_TOKEN') && WP_PROVISION_TOKEN) {
        $token = WP_PROVISION_TOKEN;
    } else {
        $token = getenv('WP_PROVISION_TOKEN');
    }
...
function jpv_portal_provision_find_header(array $headers, string $key): string {
...
function jpv_portal_provision_get_auth_header(array &$diag): string {
    $diag = array(
        'has_http_auth' => false,
        'has_redirect_http_auth' => false,
        'has_getallheaders' => false,
    );

    $header = '';

    if (!empty($_SERVER['HTTP_AUTHORIZATION'])) {
        $diag['has_http_auth'] = true;
        $header = (string) $_SERVER['HTTP_AUTHORIZATION'];
    }

    if (!empty($_SERVER['REDIRECT_HTTP_AUTHORIZATION'])) {
        $diag['has_redirect_http_auth'] = true;
        if ($header === '') {
    ...
    if (function_exists('getallheaders')) {
        ...
        if (is_array($headers)) {
            $value = jpv_portal_provision_find_header($headers, 'Authorization');
            if ($value !== '') {
                $diag['has_getallheaders'] = true;
                if ($header === '') {
                    $header = $value;
                }
            }
...
function jpv_portal_provision_extract_bearer_token(string $header): string {
    if ($header === '') {
        return '';
    }
...
function jpv_portal_provision_handle_request(WP_REST_Request $request): WP_REST_Response {
    ...
    if ($expected === '') {
        $log_data['reason'] = 'server_misconfigured';
        return jpv_portal_provision_finish(500, array(
            'ok' => false,
            'reason' => 'server_misconfigured',
            'missing' => 'WP_PROVISION_TOKEN',
        ), $log_data);
    }

    $auth_diag = array();
    $auth_header = jpv_portal_provision_get_auth_header($auth_diag);
    $provided = jpv_portal_provision_extract_bearer_token($auth_header);
    $authorized = ($provided !== '' && hash_equals($expected, $provided));

    if (!$authorized) {
        error_log('[JPV Provision/Auth] ' . wp_json_encode(array(
            'ok' => false,
            'reason' => 'unauthorized',
            'has_http_auth' => $auth_diag['has_http_auth'],
            'has_redirect_http_auth' => $auth_diag['has_redirect_http_auth'],
            'has_getallheaders' => $auth_diag['has_getallheaders'],
        )));
