...
const commentComposer = readFileSync('src/components/community/CommunityCommentComposer.tsx', 'utf8')
...
assert.match(bookmarkRoute, /toggleMemberBookmark\(payload, memberId, postId\)/)
...
assert.match(communityFileRoute, /formData\.get\('postId'\)/)
assert.match(communityFileRoute, /if \(!detail\.allowed \|\| !detail\.post\.canComment\) \{/)
assert.match(communityFileRoute, /post: String\(detail\.post\.id\)/)
assert.match(commentComposer, /data\.append\('postId', postId\)/)
