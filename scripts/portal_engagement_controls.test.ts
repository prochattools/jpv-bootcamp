...
const communityFileRoute = readFileSync('src/app/api/community/files/route.ts', 'utf8')
...
assert.match(communityFileRoute, /if \(!detail\?\.allowed\) \{/)
assert.doesNotMatch(communityFileRoute, /detail\?\.membership\?\.status !== 'active'/)
