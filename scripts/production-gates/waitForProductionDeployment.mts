import { assertProductionOrigin } from './productionPolicy'
...
const productionUrl = requireEnvironment('PRODUCTION_URL').replace(/\/$/, '')
const expectedSha = requireEnvironment('EXPECTED_DEPLOYMENT_SHA')
const maximumAttempts = 40
const delayMilliseconds = 15_000

assertProductionOrigin(productionUrl)

if (!/^[0-9a-f]{40}$/i.test(expectedSha)) {
  throw new Error(
    'PRODUCTION-DEPLOYMENT-WAIT-DENIED: EXPECTED_DEPLOYMENT_SHA must be a full 40-character commit SHA',
  )
}

for (let attempt = 1; attempt <= maximumAttempts; attempt += 1) {
  try {
    const response = await fetch(`${productionUrl}/api/health/deployment`, {
      cache: 'no-store',
      redirect: 'error',
...
throw new Error(
  `PRODUCTION-DEPLOYMENT-WAIT-FAILED: production did not serve ${expectedSha} within 10 minutes`,
)
