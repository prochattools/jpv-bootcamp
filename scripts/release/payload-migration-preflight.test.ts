import assert from 'node:assert/strict'
import { createRequire } from 'node:module'

import { PAYLOAD_MIGRATION_NAMES } from '../../src/lib/payloadMigrationRegistry'

const require = createRequire(import.meta.url)
const preflight = require('./payload-migration-preflight.cjs') as {
  REQUIRED_PAYLOAD_MIGRATIONS: string[]
  REQUIRED_AUDIT_HISTORY_COLUMNS: string[]
  REQUIRED_PAYLOAD_RELATION_TABLES: Array<{ table: string; columns: string[] }>
  missingMigrationNames(rows: Array<{ name: string }>): string[]
  resolveSchema(environment: Record<string, string | undefined>): string
  verifyPayloadMigrationState(options: {
    environment: Record<string, string | undefined>
    clientFactory: () => { connect(): Promise<void>; query(sql: string, values?: unknown[]): Promise<{ rows: Array<{ name?: string; table_name?: string; column_name?: string }> }>; end(): Promise<void> }
  }): Promise<string[]>
}

assert.deepEqual(preflight.REQUIRED_PAYLOAD_MIGRATIONS, PAYLOAD_MIGRATION_NAMES)
assert.equal(preflight.resolveSchema({ DATABASE_URL: 'postgresql://user:password@db/app?schema=jpvbootcamp_staging' }), 'jpvbootcamp_staging')
assert.equal(preflight.resolveSchema({ DATABASE_URL: 'postgresql://user:password@db/app' }), 'jpvbootcamp')
assert.equal(preflight.resolveSchema({ DATABASE_URL: 'postgresql://user:password@db/app?schema=ignored', PAYLOAD_MIGRATION_SCHEMA: 'reviewed_schema' }), 'reviewed_schema')
assert.throws(() => preflight.resolveSchema({ DATABASE_URL: 'not-a-url' }), /database_url_unavailable/)
assert.throws(() => preflight.resolveSchema({ DATABASE_URL: 'postgresql://user:password@db/app?schema=invalid-schema' }), /invalid_schema/)

const lastMigration = preflight.REQUIRED_PAYLOAD_MIGRATIONS.at(-1)
assert(lastMigration)
assert.deepEqual(preflight.missingMigrationNames([{ name: preflight.REQUIRED_PAYLOAD_MIGRATIONS[0] }]), preflight.REQUIRED_PAYLOAD_MIGRATIONS.slice(1))

async function main(): Promise<void> {
  const queries: string[] = []
  const missing = await preflight.verifyPayloadMigrationState({
    environment: { DATABASE_URL: 'postgresql://user:password@db/app?schema=jpvbootcamp_staging' },
    clientFactory: () => ({
      async connect() {},
      async query(sql: string) {
        queries.push(sql)
        if (sql.includes('payload_migrations')) {
          return { rows: preflight.REQUIRED_PAYLOAD_MIGRATIONS.filter((name) => name !== lastMigration).map((name) => ({ name })) }
        }
        return { rows: preflight.REQUIRED_AUDIT_HISTORY_COLUMNS.map((column_name) => ({ column_name })) }
      },
      async end() {},
    }),
  })
  assert.deepEqual(missing, [lastMigration])
  assert.deepEqual(queries, ['SELECT "name" FROM "jpvbootcamp_staging"."payload_migrations" WHERE "batch" <> -1'])

  const relationQueries: string[] = []
  const relationColumns = preflight.REQUIRED_PAYLOAD_RELATION_TABLES.flatMap(({ table, columns }) =>
    columns.map((column_name) => ({ table_name: table, column_name })),
  )
  const current = await preflight.verifyPayloadMigrationState({
    environment: { DATABASE_URL: 'postgresql://user:password@db/app?schema=jpvbootcamp_staging' },
    clientFactory: () => ({
      async connect() {},
      async query(sql: string) {
        relationQueries.push(sql)
        if (sql.includes('payload_migrations')) return { rows: preflight.REQUIRED_PAYLOAD_MIGRATIONS.map((name) => ({ name })) }
        if (sql.includes('ANY($2::text[])')) return { rows: relationColumns }
        return { rows: preflight.REQUIRED_AUDIT_HISTORY_COLUMNS.map((column_name) => ({ column_name })) }
      },
      async end() {},
    }),
  })
  assert.deepEqual(current, [])
  assert.equal(relationQueries.length, 3)
  assert.match(relationQueries[2], /information_schema\.columns/)

  await assert.rejects(
    preflight.verifyPayloadMigrationState({
      environment: { DATABASE_URL: 'postgresql://user:password@db/app?schema=jpvbootcamp_staging' },
      clientFactory: () => ({
        async connect() {},
        async query(sql: string) {
          if (sql.includes('payload_migrations')) return { rows: preflight.REQUIRED_PAYLOAD_MIGRATIONS.map((name) => ({ name })) }
          return { rows: [{ column_name: 'membership_support_id' }] }
        },
        async end() {},
      }),
    }),
    /audit_history_schema_incompatible/,
  )

  await assert.rejects(
    preflight.verifyPayloadMigrationState({
      environment: { DATABASE_URL: 'postgresql://user:password@db/app?schema=jpvbootcamp_staging' },
      clientFactory: () => ({
        async connect() {},
        async query(sql: string) {
          if (sql.includes('payload_migrations')) return { rows: preflight.REQUIRED_PAYLOAD_MIGRATIONS.map((name) => ({ name })) }
          if (sql.includes('ANY($2::text[])')) return { rows: [{ table_name: 'payload_pay_it_forward_funding_rels', column_name: 'id' }] }
          return { rows: preflight.REQUIRED_AUDIT_HISTORY_COLUMNS.map((column_name) => ({ column_name })) }
        },
        async end() {},
      }),
    }),
    /payload_relationship_schema_incompatible/,
  )
  console.log('payload-migration-preflight.test.ts passed')
}

main().catch((error) => {
  console.error(error)
  process.exitCode = 1
})
