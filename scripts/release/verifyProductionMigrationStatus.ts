...
export async function readProductionRevision(expectedSha: string): Promise<ProductionRevisionEvidence> {
  ...
  if (record.ok !== true || record.status !== 'live' || record.deploymentEnv !== PRODUCTION.deploymentEnv) {
    throw new Error('Production deployment health identity is invalid')
  }
  const observedCommitSha = typeof record.commitSha === 'string' && FULL_SHA.test(record.commitSha)
    ? record.commitSha
    : null
  const observedImageTag = typeof record.imageTag === 'string' && FULL_SHA.test(record.imageTag)
    ? record.imageTag
    : null
