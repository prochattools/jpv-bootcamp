...
export const RELEASE_TEST_MANIFEST: ReleaseTestEntry[] = [
  ...
  test('staging.migration-plan-workflow-contract', 'deployment boundary and staging safety', 'scripts/staging-gates/stagingPayloadMigrationPlanWorkflowContract.test.ts', 'Statically verifies staging-payload-migration-plan.yml enforces workflow_dispatch-only trigger, confirmation guard, 40-char SHA guard, feature-branch ancestry, remote-tip match, credential masking, network gate, exact read-only plan command, sanitized artifact, and absence of forbidden commands.', 'Staging migration plan workflow contract drifted — the workflow could run without full guards, leak credentials, or execute forbidden mutations.', 'readiness'),
]
