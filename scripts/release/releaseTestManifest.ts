...
export const RELEASE_TEST_MANIFEST: ReleaseTestEntry[] = [
  ...
  test('migration.production-status-read-only', 'migration inventory, readiness, rehearsal, and rollback safety', 'scripts/release/verifyProductionMigrationStatus.test.ts', 'Proves the current-main production migration ledger verifier is exact-target, deployed-revision-aware, strictly read-only, and independent of historical feature branches.', 'Production schema evidence could target staging or legacy, hide pending or unexpected migrations, or mutate the database while reporting status.', 'readiness'),
  ...
  test('staging.migration-plan-environment-configurator', 'deployment boundary and staging safety', 'scripts/staging-gates/configureStagingMigrationPlanEnvironment.test.ts', 'Proves dry-run-default protected-path configurator validates exact repo/branch/SHA, preserves the required reviewer and exact feature/*, fix/*, release/* branch policies, transports secrets via stdin only, checks guarded path cleanliness, verifies post-apply state (exact environment, protected branch policy, PLAN_READY_FOR_DISPATCH, SOLO_OPERATOR_MODE, variables, secrets), and rejects malicious input.', 'Environment configurator could accept wrong repo/branch, weaken reviewer or branch protections, leak secrets, skip post-apply verification, or accept shell-injection attempts.', 'readiness'),
]
