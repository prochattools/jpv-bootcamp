...
export const RELEASE_TEST_MANIFEST: ReleaseTestEntry[] = [
  ...
  test('staging.production-env-validator', 'deployment boundary and staging safety', 'scripts/production-gates/checkProductionDeploymentEnv.test.ts', 'Proves canonical environment validator rejects missing secrets, staging IDs, arbitrary IDs, wrong branch, short SHA — before any Dokploy network call.', 'Environment validation can be bypassed, allowing unvalidated Dokploy API calls.', 'readiness'),
  test('staging.production-deployment-wait', 'deployment boundary and staging safety', 'scripts/production-gates/productionDeploymentWait.test.ts', 'Proves production deployment wait rejects non-production URLs, invalid SHAs, handles malformed JSON, non-200 retries, network errors, and exact-match requirement — zero real network calls.', 'Production wait polling can contact wrong hosts, accept mismatched SHAs, or crash on unexpected responses.', 'readiness'),
  test('staging.production-workflow-contract', 'deployment boundary and staging safety', 'scripts/production-gates/productionWorkflowContract.test.ts', 'Statically verifies deploy.yml invokes executable canonical policy before Docker/Dokploy, enforces exact app-ID via policy module, discards response bodies, includes SHA ancestry check, and rejects arbitrary non-staging IDs.', 'Hardened production workflow contract drifted — deploy.yml could allow untested builds, skipped gates, or staging contamination.', 'readiness'),
  test('staging.migration-plan-workflow-contract', 'deployment boundary and staging safety', 'scripts/staging-gates/stagingPayloadMigrationPlanWorkflowContract.test.ts', 'Statically verifies the read-only-plan job in deploy-preview.yml: --output=json flag, mixed-output rejection, jq -e trailing-bytes check, schema type validation, enhanced credential scan (PostgreSQL URLs, userinfo, Bearer/Basic auth, credential assignments, DB env names), injected argument array for shell-injection resistance, feature-branch exact branch policy, prevent_self_review, Tailscale fail-closed, and absence of forbidden commands.', 'Staging migration plan workflow contract drifted — the workflow could mix output streams, skip schema validation, miss credential patterns, allow wildcard branch policies, permit self-review, or fail open on missing Tailscale.', 'readiness'),
  test('staging.migration-plan-environment-configurator', 'deployment boundary and staging safety', 'scripts/staging-gates/configureStagingMigrationPlanEnvironment.test.ts', 'Proves dry-run-default configurator validates exact repo/branch/SHA/caller identity, prevents self-review, resolves login→numeric-ID via GitHub REST API, passes environment/variable/branch-policy JSON via stdin (not argv), transports secrets via stdin only, checks guarded path cleanliness, verifies post-apply state (exact environment, branch policy, reviewer, prevent_self_review, variables, secrets), and rejects malicious input.', 'Environment configurator could accept wrong repo/branch/caller, permit self-review, leak secrets, fail to resolve reviewer login, skip post-apply verification, or accept shell-injection attempts.', 'readiness'),
]
