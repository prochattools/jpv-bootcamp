...
export const RELEASE_TEST_MANIFEST: ReleaseTestEntry[] = [
  ...
  test('migration.rooms-staging-rollback-preparation-control', 'migration inventory, readiness, rehearsal, and rollback safety', 'scripts/release/prepareRoomsStagingRollback.test.ts', 'Protects exact staging rollback-preparation target guards, protected dump storage, checksum evidence, disposable restore testing, and sanitized outcome handling.', 'The Rooms staging migration could proceed without a durable, integrity-checked recovery point or could expose backup data.', 'readiness'),
  test('migration.rooms-production-control', 'migration inventory, readiness, rehearsal, and rollback safety', 'scripts/release/productionRoomsMigration.test.ts', 'Protects the exact production Rooms PLAN/APPLY/finalize control, immutable migration source, historical anomaly baseline, target guards, and compatibility hold.', 'The Rooms production migration could target the wrong environment, widen migration scope, rewrite history, or expose the old runtime to an incompatible navigation state.', 'readiness'),
  ...
  test('migration.cleanup-error-precedence', 'migration inventory, readiness, rehearsal, and rollback safety', 'scripts/release/cleanupErrorPrecedence.test.ts', 'Protects cleanup error precedence so a cleanup failure cannot mask the primary operation failure, while cleanup-only failures still fail closed.', 'Release controllers can report the wrong failure or silently ignore cleanup failure.', 'readiness'),
  ...
  test('migration.production-status-read-only', 'migration inventory, readiness, rehearsal, and rollback safety', 'scripts/release/verifyProductionMigrationStatus.test.ts', 'Proves the production migration ledger verifier targets the canonical production identity, matches the deployed revision, uses a read-only transaction, and performs no migration or data mutation.', 'Production schema evidence could target staging or legacy, accept a non-live response, hide pending migrations, or mutate the database while reporting status.', 'readiness'),
  ...
  test('security.response-headers', 'authentication and account security', 'scripts/release/securityHardening.test.ts', 'Protects baseline response security headers and preserves LiveKit camera/microphone access.', 'Public, portal, API, or admin responses lose baseline browser security controls or break LiveKit media permissions.', 'M1-02'),
  ...
  command('payload.member-group-candidates', 'Payload admin, editor, collection, and access behavior', 'pnpm', ['exec', 'vitest', 'run', 'src/__tests__/member-group-candidates.test.ts'], 'Protects the administrator group audience projection and its member/administrator metadata.', 'Member groups can omit administrators, expose the wrong audience, or lose identifying metadata.', 'M1-02'),
  command('payload.portal-announcement-admin', 'Payload admin, editor, collection, and access behavior', 'pnpm', ['exec', 'vitest', 'run', 'src/__tests__/portal-announcement-commands.test.ts'], 'Protects administrator update listing and edit/archive/delete lifecycle behavior.', 'Administrators cannot safely manage all update statuses or a lifecycle action can mutate the wrong record.', 'M1-02'),
  ...
  test('stripe.mighty-entitlement-contract', 'Stripe checkout, webhook, schedule, invoice, recovery, refund, and dispute handling', 'src/lib/mighty/stripeEntitlementSummary.test.ts', 'Protects the canonical Stripe-to-Mighty ALLOWED/DENIED lifecycle, no-grace payment failure behavior, and recovery semantics.', 'Stripe lifecycle projection can grant, retain, or restore Mighty access incorrectly.', 'readiness'),
  test('stripe.mighty-access-sync', 'Stripe checkout, webhook, schedule, invoice, recovery, refund, and dispute handling', 'src/lib/mighty/accessSync.test.ts', 'Protects Mighty identity resolution, event ordering, duplicate assignment handling, Plan-only revoke, and production mutation guards.', 'Mighty reconciliation can duplicate identities, apply stale Stripe events, or mutate an unsafe identity.', 'readiness'),
  test('stripe.mighty-lifecycle-readiness', 'Stripe checkout, webhook, schedule, invoice, recovery, refund, and dispute handling', 'scripts/mighty/lifecycleReadiness.test.ts', 'Proves privileged protection, paid and coupon entitlement, new-member onboarding, payment failure/recovery, cancellation, idempotency, and exact safe email rendering through a synthetic Mighty provider.', 'A Stripe lifecycle event can revoke privileged access, mishandle discounted subscriptions, duplicate a Mighty identity, send obsolete onboarding instructions, or fail to restore access.', 'readiness'),
  command('stripe.mighty-phase-a-bootstrap', 'Stripe checkout, webhook, schedule, invoice, recovery, refund, and dispute handling', 'pnpm', ['exec', 'vitest', 'run', 'src/__tests__/mighty-bootstrap.test.ts', 'src/__tests__/mighty-bootstrap-queue.test.ts', 'src/__tests__/mighty-bootstrap-route.test.ts'], 'Protects the exact-account Phase A Stripe-truth queue bootstrap, provenance fields, idempotence, route authorization, and zero-provider-mutation boundary.', 'The operator bootstrap can accept an unauthorized identity, fabricate a Stripe event, lose ordering, or bypass the existing worker boundary.', 'readiness'),
  test('stripe.mighty-event-ordering', 'Stripe checkout, webhook, schedule, invoice, recovery, refund, and dispute handling', 'src/lib/mighty/eventMapping.test.ts', 'Protects replay, out-of-order, payment-recovery, and ended-subscription event ordering decisions.', 'Reordered or replayed Stripe events can restore access incorrectly or suppress a valid recovery.', 'readiness'),
  test('stripe.mighty-provider-contract', 'Stripe checkout, webhook, schedule, invoice, recovery, refund, and dispute handling', 'src/lib/mighty/adminApi.test.ts', 'Protects the Mighty Plan membership API contract and narrow duplicate-422 interpretation.', 'Provider API changes can silently turn a duplicate or revoke into incorrect access state.', 'readiness'),
  test('stripe.mighty-inert-deployment-safety', 'Stripe checkout, webhook, schedule, invoice, recovery, refund, and dispute handling', 'scripts/mighty/inertDeploymentSafety.test.ts', 'Proves an application deployment does not start population work, that Stripe webhooks remain queue-only, and that scheduler/worker controls stay fail-closed.', 'A deployed application could start Mighty population activity or allow an unauthorized worker path.', 'readiness'),
  test('stripe.mighty-mutation-policy', 'Stripe checkout, webhook, schedule, invoice, recovery, refund, and dispute handling', 'src/lib/mighty/mutationPolicy.test.ts', 'Protects explicit production allowlists and fail-closed host, administrator, exception, and unresolved-role safeguards.', 'A live Stripe-derived mutation can reach an unauthorized or privileged Mighty identity.', 'readiness'),
  test('stripe.mighty-reconciliation-dry-run', 'Stripe checkout, webhook, schedule, invoice, recovery, refund, and dispute handling', 'src/lib/mighty/reconciliation.test.ts', 'Protects read-only reconciliation classifications for target Plan drift, privilege exclusions, overlaps, and provider errors.', 'A reconciliation report can propose unsafe actions or hide overlapping access.', 'readiness'),
  test('stripe.mighty-cutover-manifest', 'Stripe checkout, webhook, schedule, invoice, recovery, refund, and dispute handling', 'src/lib/mighty/cutoverManifest.test.ts', 'Protects the complete per-identity Stripe-to-Mighty cutover manifest and fail-closed role, identity, and overlap action classes.', 'The migration manifest can classify a real identity as safe when its provider role or access overlap is unresolved.', 'readiness'),
  test('stripe.mighty-cutover-runner', 'Stripe checkout, webhook, schedule, invoice, recovery, refund, and dispute handling', 'src/lib/mighty/cutoverRunner.test.ts', 'Protects bounded batch execution, checkpoint/resume, idempotent rerun, stop-on-error, uncertain provider handling, and review skips with synthetic provider adapters.', 'A future cutover batch can continue after an error, repeat a mutation, or bypass an identity review.', 'readiness'),
  test('stripe.mighty-zero-touch-operations', 'Stripe checkout, webhook, schedule, invoice, recovery, refund, and dispute handling', 'scripts/mighty/migrationReadiness.test.ts', 'Protects exact manifest hashing, owner authorization, worker authentication, entitlement re-checks, explicit dry-run/apply mode, bounded batches, overlap and privilege stops, and synthetic operator rehearsal.', 'A future migration can start without exact approval, trust stale entitlement, discover a population, or continue after an ambiguity or provider uncertainty.', 'readiness'),
  test('stripe.mighty-synthetic-hardening', 'Stripe checkout, webhook, schedule, invoice, recovery, refund, and dispute handling', 'scripts/mighty/syntheticHardening.test.ts', 'Proves the complete synthetic member, entitlement, provider-failure, and test-data ownership matrices without live provider access.', 'A hardening scenario can silently fall back to live data, misclassify overlap or privilege, or clean up data it did not create.', 'readiness'),
  test('stripe.mighty-cutover-rehearsal', 'Stripe checkout, webhook, schedule, invoice, recovery, refund, and dispute handling', 'scripts/mighty/rehearseCutover.test.ts', 'Protects the complete production-shaped live Stripe/Mighty read-only rehearsal and its no-mutation boundary.', 'The pre-cutover rehearsal can silently become a partial local-roster audit or call a provider mutation.', 'readiness'),
  test('stripe.mighty-migration-contract', 'Stripe checkout, webhook, schedule, invoice, recovery, refund, and dispute handling', 'scripts/mighty_migration_contract.test.ts', 'Protects the complete Stripe-to-Mighty migration contract and keeps production acceptance bounded to authorized test identities.', 'The provider acceptance boundary, entitlement authority, or production safety documentation can drift.', 'readiness'),
  ...
  command('email.mighty-migration-silence', 'email queue, retry, and redaction behavior', 'pnpm', ['exec', 'vitest', 'run', 'src/__tests__/email-outbox-behavioral.test.ts'], 'Proves the hard-coded Mighty migration recipient policy at enqueue and final provider boundaries.', 'A migration/cutover path can create or send unauthorized email, or retry a suppressed event.', 'readiness'),
  ...
  test('rooms.creator-invitation', 'member portal and entitlement behavior', 'scripts/room_creator_invitation.test.ts', 'Proves every Room creator is included in durable access, invitation email, and member notification reconciliation, including legacy creator visibility without a data rewrite.', 'A Room creator can be omitted from member access, notifications, or invitation email delivery.', 'M1-02'),
