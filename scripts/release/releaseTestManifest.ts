...
export const RELEASE_TEST_MANIFEST: ReleaseTestEntry[] = [
  ...
  test('stripe.webhook-toctou', 'Stripe checkout, webhook, schedule, invoice, recovery, refund, and dispute handling', 'src/__tests__/stripe-webhook-toctou.test.ts', 'Proves handleStripeWebhook() deduplicates concurrent identical events, releases the idempotency claim on handler failure, and allows retry — structural guarantee that markProcessed is never called in the catch path.', 'Concurrent Stripe webhook delivery can double-provision or permanently block retries after a transient failure.', 'M0-05', ['M0-05']),
