...
const startup = read('scripts/release/start-staging.sh')
...
assert.equal(packageJson.scripts?.start, 'scripts/release/start-staging.sh')
...
assert.match(dockerfile, /CMD \["bash", "scripts\/release\/start-staging\.sh"\]/)
