...
function main() {
  const prices = {
    pricePro: 'price_membership_monthly',
    priceProAnnual: 'price_membership_annual',
  }

  assert.equal(parseCheckoutPlan('membership'), 'membership')
  assert.equal(parseCheckoutPlan('MEMBERSHIP'), 'membership')
  assert.equal(parseCheckoutPlan(' jpv_bootcamp_membership '), 'membership')
  assert.equal(parseCheckoutPlan(null), null)
  assert.equal(parseCheckoutPlan(''), null)
  assert.equal(parseCheckoutPlan('pro'), null)
  assert.equal(parseCheckoutPlan('free'), null)
  ...
  assert.equal(getCheckoutPriceId('membership', 'monthly', prices), 'price_membership_monthly')
  assert.equal(getCheckoutPriceId('membership', 'annual', prices), 'price_membership_annual')
