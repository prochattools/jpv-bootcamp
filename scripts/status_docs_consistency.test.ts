...
async function main(): Promise<void> {
  ...
  assert.match(docs.roadmap, /`pnpm test:release` passed `121\/121`/)
  ...
  assert.match(docs.operatorHandoff, /Deterministic non-browser release gate: `pnpm test:release` \(`121\/121`\)/)
  ...
  assert.match(docs.previewReadiness, /`pnpm test:release` passed `121\/121`/)
