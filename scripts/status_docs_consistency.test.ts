...
async function main(): Promise<void> {
  ...
  assert.match(docs.roadmap, /ACCOUNT-ACTION HARDENING IMPLEMENTED LOCALLY — STAGING MIGRATION AUTHORIZATION REQUIRED/)
  assert.match(docs.roadmap, /29-name registry/)
  ...
  assert.match(docs.operatorHandoff, /ACCOUNT-ACTION HARDENING IMPLEMENTED LOCALLY — STAGING MIGRATION AUTHORIZATION REQUIRED/)
  ...
  assert.match(docs.operatorHandoff, /Migration inventory.*29\/29|Migrations applied.*No/i)
  ...
  assert.match(docs.previewReadiness, /ACCOUNT-ACTION HARDENING IMPLEMENTED LOCALLY — STAGING MIGRATION AUTHORIZATION REQUIRED/)
  ...
  assert.match(docs.previewReadiness, /LOCAL HARDENING COMPLETE — CONTROLLED STAGING MIGRATION APPROVAL REQUIRED/)
  assert.match(docs.previewReadiness, /DECISION-READY, EXTERNAL APPROVALS PENDING/)
  assert.match(docs.previewReadiness, /canonical 29-name Payload registry/)
  ...
  for (const currentDoc of [docs.previewReadiness, docs.roadmap, docs.operatorHandoff]) {
    ...
    assert.match(currentDoc, /account-action[\s\S]{0,180}reservation\/finalization|reservation\/finalization[\s\S]{0,180}account-action/i)
