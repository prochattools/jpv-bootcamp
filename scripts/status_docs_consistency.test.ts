...
async function main(): Promise<void> {
  ...
  assert.match(docs.roadmap, /Provider\/email acceptance \| Pending \|/)
  assert.match(docs.roadmap, /Complete staging\/browser smoke \| Pending \|/)
  assert.match(docs.roadmap, /~68%/)
  assert.match(docs.roadmap, /~82%/)
  assert.match(docs.roadmap, /~86%/)
  assert.match(docs.roadmap, /~76%/)
  assert.match(docs.roadmap, /~55%/)
  assert.match(docs.roadmap, /~20%/)
  assert.match(docs.roadmap, /audited against commit `236227c fix: require portal auth for member content`/)
  assert.match(docs.roadmap, /Static preflight runs a subset/)
  assert.match(docs.roadmap, /No Playwright\/Cypress E2E configuration/)

  assert.match(docs.operatorHandoff, /Version 3\.5 current client go-live plan; Version 3\.4 is the prior progress baseline/)
  assert.match(docs.operatorHandoff, /Last audited baseline: `236227c fix: require portal auth for member content`/)
