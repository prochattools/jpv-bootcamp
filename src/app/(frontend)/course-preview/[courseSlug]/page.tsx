          ...
          <div className='grid lg:grid-cols-[1.25fr_0.75fr]'>
            <div className='p-8 sm:p-10 lg:p-14'>
              <div className='flex flex-wrap items-center gap-3'>
                <span className='rounded-full bg-[var(--jpv-sunshine)] px-3 py-1 text-xs font-extrabold tracking-[0.15em] text-[var(--jpv-brand-deep)]'>PRO</span>
                <span className='rounded-full border border-white/20 px-3 py-1 text-xs font-semibold text-[var(--jpv-inverse-muted)]'>9 lessons</span>
                <span className='rounded-full border border-white/20 px-3 py-1 text-xs font-semibold text-[var(--jpv-inverse-muted)]'>3h 20m</span>
              </div>
              <p className='mt-8 text-sm font-bold uppercase tracking-[0.2em] text-[var(--jpv-sunshine)]'>Course overview</p>
              ...
              <div className='mt-9 flex flex-wrap gap-4'>
                <Link href={`/course-preview/${courseSlug}/how-to-use-the-programme`} className='rounded-full bg-[var(--jpv-sunshine)] px-6 py-3 text-sm font-bold text-[var(--jpv-brand-deep)] no-underline'>
                  Continue course
                </Link>
            ...
            <div className='border-t border-white/10 bg-[var(--jpv-brand-hover)] p-8 lg:border-l lg:border-t-0 lg:p-10'>
              <p className='text-xs font-bold uppercase tracking-[0.18em] text-[var(--jpv-sunshine)]'>Your progress</p>
              <div className='mt-6 flex items-end justify-between'>
                <div>
                  <p className='text-5xl font-bold'>36%</p>
                  <p className='mt-2 text-sm text-[var(--jpv-inverse-muted)]'>3 of 9 lessons viewed</p>
                </div>
                <div className='flex h-20 w-20 items-center justify-center rounded-full border-[7px] border-[var(--jpv-sunshine)] text-sm font-bold'>3 / 9</div>
              </div>
              <div className='mt-8 h-2 overflow-hidden rounded-full bg-white/10'>
                <div className='h-full w-[36%] rounded-full bg-[var(--jpv-sunshine)]' />
              </div>
              <div className='mt-8 rounded-2xl border border-white/10 bg-white/[0.04] p-5'>
                <p className='text-xs font-semibold uppercase tracking-[0.16em] text-[var(--jpv-inverse-muted)]'>Next lesson</p>
                <p className='mt-2 font-bold'>How to Use the Programme</p>
                <p className='mt-1 text-sm text-[var(--jpv-inverse-muted)]'>11 minutes</p>
              </div>
            </div>
          </div>
        </section>
        ...
        <section className='mt-14 grid gap-8 lg:grid-cols-[1fr_330px]'>
          <div>
            <div className='flex items-end justify-between gap-4'>
              <div>
                <p className='text-xs font-bold uppercase tracking-[0.2em] text-[var(--jpv-sunshine-ink)]'>Curriculum</p>
                <h2 className='mt-2 text-3xl font-bold tracking-tight text-[var(--jpv-brand-deep)]'>Course structure</h2>
              </div>
              <span className='text-sm font-semibold text-[var(--jpv-muted)]'>3 modules</span>
            </div>

            <div className='mt-7 space-y-5'>
              {modules.map((module, moduleIndex) => (
                <article key={module.title} className='overflow-hidden rounded-jpv-card border border-[var(--jpv-brand-deep)]/10 bg-white shadow-jpv-card'>
                  <div className='border-b border-[var(--jpv-brand-deep)]/8 bg-[var(--jpv-canvas)] px-6 py-5 sm:px-7'>
                    <div className='flex flex-col justify-between gap-3 sm:flex-row sm:items-center'>
                      <div>
                        <p className='text-xs font-bold uppercase tracking-[0.17em] text-[var(--jpv-sunshine-ink)]'>Module {moduleIndex + 1}</p>
                        <h3 className='mt-1 text-xl font-bold text-[var(--jpv-brand-deep)]'>{module.title}</h3>
                        <p className='mt-2 text-sm text-[var(--jpv-muted)]'>{module.description}</p>
                      </div>
                      <span className='shrink-0 rounded-full bg-[var(--jpv-surface-strong)] px-3 py-2 text-xs font-bold text-[var(--jpv-muted)]'>{module.lessons.length} lessons</span>
                    </div>
                  </div>
                    ...
                    {module.lessons.map((lesson, lessonIndex) => (
                      <div key={lesson.title} className='flex flex-col justify-between gap-4 px-6 py-5 sm:flex-row sm:items-center sm:px-7'>
                        <div className='flex items-center gap-4'>
                          <span className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-full text-sm font-bold ${lesson.state === 'completed' ? 'bg-[var(--jpv-sunshine)] text-[var(--jpv-brand-deep)]' : 'bg-[var(--jpv-surface-strong)] text-[var(--jpv-muted)]'}`}>
                            {lesson.state === 'completed' ? '✓' : lessonIndex + 1}
                          </span>
                          <div>
                            <p className='font-semibold text-[var(--jpv-brand-deep)]'>{lesson.title}</p>
                            <p className='mt-1 text-xs text-[var(--jpv-muted)]'>{lesson.duration}</p>
                          </div>
                        </div>
          ...
          <aside className='space-y-6'>
            <div className='rounded-jpv-card bg-[var(--jpv-sunshine)] p-7 text-[var(--jpv-brand-deep)]'>
              ...
              <p className='mt-3 text-sm leading-6 text-[var(--jpv-muted)]'>By the end of this course, members should understand their audience, offer and next practical step.</p>
            </div>

            <div className='rounded-jpv-card border border-[var(--jpv-brand-deep)]/10 bg-white p-7'>
              <p className='text-xs font-bold uppercase tracking-[0.18em] text-[var(--jpv-sunshine-ink)]'>Included</p>
              <ul className='mt-5 space-y-3 text-sm text-[var(--jpv-muted)]'>
            ...
            <div className='rounded-jpv-card border border-dashed border-[var(--jpv-brand-deep)]/20 bg-[var(--jpv-canvas)] p-7'>
              <p className='text-xs font-bold uppercase tracking-[0.18em] text-[var(--jpv-sunshine-ink)]'>Prototype note</p>
              <p className='mt-3 text-sm leading-6 text-[var(--jpv-muted)]'>This screen uses static demonstration content. No member permissions, progress or live course records are connected.</p>
            </div>
          </aside>
        </section>
      </main>
