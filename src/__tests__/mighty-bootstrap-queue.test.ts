...
describe('operator bootstrap queue persistence', () => {
	...
	it('forces an existing row to remain email-silent during operator bootstrap', async () => {
		rows.push({
			id: 'sync-existing',
			email: 'westhoek@hotmail.com',
			normalizedEmail: 'westhoek@hotmail.com',
			stripeCustomerId: 'cus_synthetic',
			stripeSubscriptionId: 'sub_synthetic',
			lastStripeEventId: null,
			lastStripeEventCreatedAt: null,
			lastStripeEventType: null,
			stateSource: 'operator_bootstrap',
			stateObservedAt: new Date('2026-09-14T11:00:00.000Z'),
			plan: 'jpv_bootcamp_membership',
			desiredAccess: 'DENIED',
		...
		expect(result).toMatchObject({ queued: true, rowId: 'sync-existing' })
		expect(rows[0]).toMatchObject({ welcomeRequired: false, stateSource: 'operator_bootstrap' })
	})
})
