...
export type SponsoredDecisionAction = 'approve' | 'reject'

export type SponsoredDecisionPayload = {
	applicationId: string
	action: SponsoredDecisionAction
	iat: number
	exp: number
	nonce: string
}

export type SponsoredDecisionResult =
	| { ok: true; payload: SponsoredDecisionPayload }
	| {
			ok: false
			reason:
				| 'missing'
...
export function signSponsoredDecisionToken(
	payload: SponsoredDecisionPayload,
	secret: string
): string {
...
export function verifySponsoredDecisionToken(
	rawToken: string,
	secret: string,
	...
	const action =
		payload.action === 'approve' || payload.action === 'reject'
			? (payload.action as SponsoredDecisionAction)
			: null
	if (!action) {
		return { ok: false, reason: 'invalid_payload' }
	}
